...
export function RecruitmentAlerts() {
  ...
  const { data: institutions = [], isLoading } = useGetInstitutionAlerts({ query: { enabled: open } });
                        ...
                        <div className="flex items-start justify-between gap-3 mb-3">
                          <div className="min-w-0">
                            <div className="flex items-center gap-2 flex-wrap mb-1">
                              <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${TYPE_COLORS[inst.type] ?? "bg-slate-100 text-slate-600"}`}>
                                {inst.type}
                              </span>
                              {isUrgent && (
